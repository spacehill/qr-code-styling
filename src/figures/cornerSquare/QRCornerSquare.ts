  ...
  _basicDottedSquare(args: BasicFigureDrawArgs): void {
    ...
    this._rotateFigure({
      ...
      draw: () => {
        ...
        // Right edge
        for (let i = 0; i <= size - squareSize; i += gap) {
          pathData += `M ${x + size - squareSize} ${y + i
            } h ${squareSize} v ${squareSize} h -${squareSize} z `;
        }
        // Bottom edge
        for (let i = 0; i <= size - squareSize; i += gap) {
          pathData += `M ${x + size - i - squareSize} ${y + size - squareSize
            } h ${squareSize} v ${squareSize} h -${squareSize} z `;
        }
        // Left edge
        for (let i = 0; i <= size - squareSize; i += gap) {
          pathData += `M ${x} ${y + size - i - squareSize
            } h ${squareSize} v ${squareSize} h -${squareSize} z `;
        }
        // Top-left corner
        ...
        // Bottom-right corner
        pathData += `M ${x + size - squareSize} ${y + size - squareSize
          } h ${squareSize} v ${squareSize} h -${squareSize} z `;
        // Bottom-left corner
        pathData += `M ${x} ${y + size - squareSize
          } h ${squareSize} v ${squareSize} h -${squareSize} z `;
